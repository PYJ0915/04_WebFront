...
var menu = "초밥";
console.log("menu : ", menu);
// 변수명이 중복되어 값이 덮어쓰기 되면 이전에 저장한 변수값을 재사용할 수 없게 됨!

// 2. let (변수, var의 변수명 중복 문제 해결)

let number3 = 25;

// let number3 = 38; => 변수명 중복 선언 불가!
...
// 상수로 배정되어 재대입할 수 없는데 재대입을 하고 있어 오류가 발생!

// const test; 불가능! => 상수는 선언과 동시에 초기화 반드시!
...
// 즉, 코드 블록 내부에서 선언된 변수 (let, const)는 해당 블록 외부에서 접근 불가!
...
console.log(foo1); // 전역 변수이기 때문에 (블록에 관계없이) 접근 가능! 
console.log(foo2); // 전역 변수이기 때문에 (블록에 관계없이) 접근 가능! 
// console.log(num1);
// console.log(num2);
// => 둘 다 블록 레벨 스코프이기 때문에 블록 밖에서 접근 불가!
...
console.log(test2);
// => test2 출력 가능!
// why? 함수레벨 scope인 var는 일반 블록을 무시하기 때문!
...
// console.log(tset3);
// ReferenceError: tset3 is not defined
// => var는 일반 레벨 블록만 무시할 뿐, 함수 레벨 블록은 무시할 수 없음
// 즉, 함수 블록 안에서 선언된 var 변수는 함수 안에서만 사용가능함!

function example2() { // 함수 블록 (함수 레벨 scope)

  if(true) { // 일반 블록 (블록 레벨 scope)
    var x = 10;
  }
...
// 내부적으로 var a; 이렇게 선언만 된 상태 => 에러가 나지 않고 undefined가 찍힘
// -> 호이스팅 되어 var a; 선언부는 최상단으로 올라감.
// -> 선언은 되어 있으나 a에 값이 대입되진 않았기 때문에 undefined가 되는 것

console.log(a); // undefined

var a = 10; // 전역 scope에 선언 + 초기화

// 2. let / const의 호이스팅
// : let/const로 선언된 변수는 선언만 해당 스코프의 최상단으로 호이스팅됨.
// => var와 달리, let/const로 선언된 변수는 초기화가 이루어지기 전까지는 사용할 수 없음!
...
// 내부적으로 let x; 선언은 끌어올려진 상태 => x의 선언이 호이스팅됨.
// 그러나 TDZ에 놓여 초기화 전까지 접근 불가!
// -> 초기화가 이루어지기 전까지 해당 변수가 TDZ에 놓이며, 
//    이 기간동안 변수에 접근하려고 하면 오류 발생!
...
// => 이러한 특성들 때문에 let/const가 var보다 예측 가능한 코드 작성 가능 thus 오늘날 let/const 지향

//----------------------------------------------------------------------------------------

// JS 자료형 확인하기
// type of 연산자 : 변수/값의 자료형을 출력해주는 연산자


// undefined : 정의되지않은 변수 / 값이 아직 대입되지않았다
let undef;
...
const gender = 'M'; // 홑따옴표 & 한 글자만 작성해도 문자열(string)
console.log("gender : ", gender, typeof gender);

// number (정수, 실수, 양수, 음수 등 모든 숫자)
const age = 25;
const height = 150.5
const eyesight = -5;
...
console.log("isFlase : ", isFalse, typeof isFalse);
...
console.log("numbers 배열 값 중 3번째: ", numbers[3], typeof numbers[3]); // 크기가 자동으로 늘어남 => 값이 대입 되지 않아 정의 X
// JS에서의 배열은 값 추가 및 삭제 가능! (크기가 고정되지 않고 자동으로 늘어나거나 줄어듦)
...
const user = { id : "user01", pw : "pass01", userName : "홍기르동", age : 20 }

console.log("user : ", user, typeof user);

// 객체에 존재하는 값을 하나씩 얻어오기
// 방법 1: 변수명['key'] => 대괄호 안 문자열로 작성!
